...
two or more lines with the same length, the customer will always choose the window with the smallest number. 
...
For example, suppose that a bank has 2 windows and each window may have 2 customers waiting inside 
the yellow line. There are 5 customers waiting with transactions taking 1, 2, 6, 4 and 3 minutes, respectively. 
...
integers: N (<=20, number of windows), M (<=10, the maximum capacity of each line inside the yellow line), 
K (<=1000, number of customers), and Q (<=1000, number of customer queries).
The next line contains K positive integers, which are the processing time of the K customers.
The last line contains Q positive integers, which represent the customers who are asking about 
the time they can have their transactions done. The customers are numbered from 1 to K.
...
closed everyday after 17:00, for those customers who cannot be served before 17:00, you must output "Sorry" instead.
...
struct Customer{
	int process;
	int finish;
	Customer():process(INF),finish(INF) {}
};
...
int main()
{
	...
	vector<Customer> people(K);
	
	for(int i=0;i<K;i++)
		cin>>people[i].process;
		
	vector<queue<int> > window(N);
	vector<int> begin_time(N,0);
	
	int p;
	for(p=0; p < N*M && p<K ; p++)
	{
		people[p].finish = begin_time[p%N] + people[p].process;
		begin_time[p%N] = people[p].finish;
		window[p%N].push(p);
	}
	...
	for( ; p<K;p++)
	{
		...
		for(int j=0;j<N;j++)
		{
			...
			if(people[top].finish < min)
			{
				w = j;
				min = people[top].finish;
			}
		}
		people[p].finish = begin_time[w] + people[p].process;
		begin_time[w] = people[p].finish;
		window[w].pop();
		window[w].push(p);
	}
	...
	for(int i=0;i<Q;i++)
	{
		cin>>query;
		query--;
		start = people[query].finish - people[query].process;
		if(start >= 540) cout<<"Sorry"<<endl;
		else {
			hour = people[query].finish/60+8;
			minute = people[query].finish%60;
